...
/* BEGIN KYTY */
  ...
  RijndaelInit(aesCtx, RIJNDAEL_Direction_Mode_CBC, direction, pagekey, RIJNDAEL_Direction_KeyLength_Key16Bytes, initial, 0);
/* END KYTY */
  if (encrypt)
  {
...
/* BEGIN KYTY */
  ...
  RijndaelInit(aesCtx, RIJNDAEL_Direction_Mode_CBC, direction, pagekey, RIJNDAEL_Direction_KeyLength_Key32Bytes, initial, 0);
/* END KYTY */
  if (encrypt)
  {
    len = RijndaelBlockEncrypt(aesCtx, datain, datalen*8, dataout);
...
CloneSQLCipherCipher(void* cipherTo, void* cipherFrom)
{
  ...
  /* BEGIN KYTY */
  sqlCipherCipherTo->m_hmacEmpty = sqlCipherCipherFrom->m_hmacEmpty;
  /* END KYTY */
...
/* BEGIN KYTY */
static void
GenerateHmacSQLCipherCipher(SQLCipherCipher* sqlCipherCipher)
{
	int j;
	...
	memcpy(m_key, sqlCipherCipher->m_key, KEYLENGTH_SQLCIPHER);

    for(unsigned char i = 0; i < 7; i++)
    {
  	  m_key[ 0+i] ^= (21 >> i) | (21 << (8-i));
  	  m_key[ 8+i] ^= (21 >> i) | (21 << (8-i));
  	  m_key[16+i] ^= (21 >> i) | (21 << (8-i));
  	  m_key[24+i] ^= (21 >> i) | (21 << (8-i));
    }
	...
	switch (sqlCipherCipher->m_hmacAlgorithm)
	{
	  ...
	  case SQLCIPHER_HMAC_ALGORITHM_SHA512:
	  default:
		fastpbkdf2_hmac_sha512(m_key, KEYLENGTH_SQLCIPHER,
							   hmacSalt, SALTLENGTH_SQLCIPHER,
							   sqlCipherCipher->m_fastKdfIter,
							   sqlCipherCipher->m_hmacKey, KEYLENGTH_SQLCIPHER);
		break;
	...
	sqlCipherCipher->m_hmacEmpty = 0;
}
static unsigned char*
GetKeySQLCipherCipher(void* cipher)
{
  SQLCipherCipher* sqlCipherCipher = (SQLCipherCipher*) cipher;
  return sqlCipherCipher->m_key;
}
static void
SetKeySQLCipherCipher(void* cipher, const unsigned char* key)
{
  SQLCipherCipher* sqlCipherCipher = (SQLCipherCipher*) cipher;
  memcpy(sqlCipherCipher->m_key, key, KEYLENGTH_SQLCIPHER);
  sqlCipherCipher->m_hmacEmpty = 1;
}
/* END KYTY */
...
/* BEGIN KYTY */
    if (sqlCipherCipher->m_hmacUse != 0) sqlCipherCipher->m_hmacEmpty = 0;
    for(unsigned char i = 0; i < 7; i++)
    {
  	  sqlCipherCipher->m_key[ 0+i] ^= (21 >> i) | (21 << (8-i));
  	  sqlCipherCipher->m_key[ 8+i] ^= (21 >> i) | (21 << (8-i));
  	  sqlCipherCipher->m_key[16+i] ^= (21 >> i) | (21 << (8-i));
  	  sqlCipherCipher->m_key[24+i] ^= (21 >> i) | (21 << (8-i));
    }
/* END KYTY */
...
/* BEGIN KYTY */
  ...
  RijndaelInit(sqlCipherCipher->m_aes, RIJNDAEL_Direction_Mode_CBC, RIJNDAEL_Direction_Encrypt, sqlCipherCipher->m_key, RIJNDAEL_Direction_KeyLength_Key32Bytes, iv, 1);
/* END KYTY */
...
/* BEGIN KYTY */
    if (sqlCipherCipher->m_hmacEmpty == 1) GenerateHmacSQLCipherCipher(sqlCipherCipher);
/* END KYTY */
...
/* BEGIN KYTY */
    ...
    RijndaelInit(sqlCipherCipher->m_aes, RIJNDAEL_Direction_Mode_CBC, RIJNDAEL_Direction_Decrypt, sqlCipherCipher->m_key, RIJNDAEL_Direction_KeyLength_Key32Bytes, iv, 1);
/* END KYTY */
...
/* BEGIN KYTY */
typedef unsigned char* (*GetKey_t)(void* cipher);
typedef void (*SetKey_t)(void* cipher, const unsigned char* key);
static unsigned char* GetKeyNullCipher(void* cipher) { return NULL; }
static void SetKeyNullCipher(void* cipher, const unsigned char* key) {  }
/* END KYTY */
...
/* BEGIN KYTY */
				 GetKeySQLCipherCipher,
				 SetKeySQLCipherCipher,
/* END KYTY */
...
/* BEGIN KYTY */
static unsigned char* CodecGetKeyWriteCipher(Codec* codec);
static void CodecSetKeyWriteCipher(Codec* codec, const unsigned char*);
static void CodecSetKeyReadCipher(Codec* codec, const unsigned char*);

void
wxsqlite3_codec_data_set(sqlite3* db, const char* zDbName, const char* paramName, const unsigned char *data)
{
  if (db != NULL && paramName != NULL)
  {
    int iDb = (zDbName != NULL) ? sqlite3FindDbName(db, zDbName) : 0;

    if ((sqlite3_stricmp(paramName, "cipher_key") == 0) && (iDb >= 0))
    {
      ...
      {
        CodecSetKeyWriteCipher(codec, data);
      }
      if (codec != NULL && CodecIsEncrypted(codec) && CodecHasReadCipher(codec))
      {
        CodecSetKeyReadCipher(codec, data);
      }
    }
  }
}
...
/* BEGIN KYTY */
    if ((sqlite3_stricmp(paramName, "cipher_key") == 0) && (iDb >= 0))
    {
      ...
      if (codec != NULL && CodecIsEncrypted(codec) && CodecHasWriteCipher(codec))
      {
        unsigned char* key = CodecGetKeyWriteCipher(codec);
        if (key != NULL)
        {
          if (!toRaw)
          {
            int j;
            result = sqlite3_malloc(KEYLENGTH_SQLCIPHER*2 + 1);
            for (j = 0; j < KEYLENGTH_SQLCIPHER; ++j)
            {
              result[j * 2] = hexdigits[(key[j] >> 4) & 0x0F];
              result[j * 2 + 1] = hexdigits[(key[j]) & 0x0F];
            }
            result[KEYLENGTH_SQLCIPHER*2] = '\0';
          }
          else
          {
            result = sqlite3_malloc(KEYLENGTH_SQLCIPHER + 1);
            memcpy(result, key, KEYLENGTH_SQLCIPHER);
            result[KEYLENGTH_SQLCIPHER] = '\0';
          }
        }
      }
    }
...
/* BEGIN KYTY */
static unsigned char*
CodecGetKeyWriteCipher(Codec* codec)
{
  unsigned char* key = (codec->m_hasWriteCipher && codec->m_writeCipher != NULL) ? codecDescriptorTable[codec->m_writeCipherType - 1].m_getKey(codec->m_writeCipher) : NULL;
  return key;
}
static void
CodecSetKeyWriteCipher(Codec* codec, const unsigned char *key)
{
  if (codec->m_hasWriteCipher && codec->m_writeCipher != NULL) codecDescriptorTable[codec->m_writeCipherType - 1].m_setKey(codec->m_writeCipher, key);
}
static void
CodecSetKeyReadCipher(Codec* codec, const unsigned char *key)
{
  if (codec->m_hasReadCipher && codec->m_readCipher != NULL) codecDescriptorTable[codec->m_readCipherType - 1].m_setKey(codec->m_readCipher, key);
}
/* END KYTY */
